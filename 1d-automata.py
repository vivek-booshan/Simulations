...
class Elementary(Automata):
    ...
    __slots__ = 'transition_rule'

    def __init__(self, rule_num):
        super().__init__(rule_num)
        ...
        Automata.transition_rule = {i:j for i, j in zip(configurations, rule)}
    ...
    def update(self, row):
        ...
        for i, _ in enumerate(row):
            if (i != 0) and (i <= self.dim-2):
                transition = Automata.transition_rule[tuple(row[i-1:i+2])]
                new[i] = transition
            elif i == self.dim-1:
                new[-1] = row[-1]
...
if __name__ == '__main__':
    ...
    while cont:
        ...
        dim = 256
        a.matrix(dim)
        # ris = np.random.randint(3, size=dim)
        #a.initial_state(ris)
        a.evolve_matrix()
        a.show_evolution()
        ...
        cont = int(input("Any input to continue. Otherwise press Enter or 0: "))